...
/* EN / EL language toggle: swaps [data-lang] content via the `hidden` attribute
   and persists the choice in localStorage. No page reload, no separate URLs —
   translations for every string live inline in each page's HTML. */
function initLanguageToggle() {
  const STORAGE_KEY = "site-lang";
  const TRANSLATED_ATTRS = { alt: "data-el-alt", "aria-label": "data-el-aria-label", placeholder: "data-el-placeholder", title: "data-el-title" };
  ...
  function applyLanguage(lang) {
    ...
    Object.entries(TRANSLATED_ATTRS).forEach(([attr, elAttr]) => {
      document.querySelectorAll(`[${elAttr}]`).forEach((el) => {
        ...
        const value = lang === "el" ? el.getAttribute(elAttr) : el.getAttribute(enCacheAttr);
        if (value !== null) el.setAttribute(attr, value);
      });
    });

    /* For elements that can't contain child markup (e.g. <option>), swap textContent directly */
    document.querySelectorAll("[data-el-text]").forEach((el) => {
      ...
      el.textContent = lang === "el" ? el.getAttribute("data-el-text") : el.getAttribute("data-en-cache-text");
    });
...
function initArticleFilters() {
  ...
  if (requestedTopic) {
    const matchingChip = filterBar.querySelector(`.filter-chip[data-filter="${CSS.escape(requestedTopic)}"]`);
    if (matchingChip) {
      filterBar.querySelectorAll(".filter-chip").forEach((c) => c.classList.remove("is-active"));
...
function applyFilters() {
  ...
  if (resultsMeta) {
    ...
    resultsMeta.textContent = lang === "el"
...
function initForms() {
  const FORM_STATUS_MESSAGES = {
    newsletter: {
      ...
      el: "Ευχαριστούμε — πρόκειται για προσωρινή λειτουργία. Συνδέστε έναν πάροχο email για ενεργοποίηση των εγγραφών.",
    },
    contact: {
...
/* Wire real share-intent URLs onto [data-share] links using this page's own URL/title.
   Kept out of the HTML so every article automatically gets a correct, working link
   instead of a static href="#" placeholder. */
